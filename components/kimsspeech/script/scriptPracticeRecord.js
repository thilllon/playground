...
export const ScriptPracticeRecord = (props) => {
  useEffect(() => {
  ...
  let sentenceRef = useRef(null);
  let sentenceIDRef = useRef(null);
  let scriptIDRef = useRef(null);
  let scriptLanRef = useRef(null);
  const handleOpen = () => {
    setOpen(true);
  };
  ...
  return (
    <div>
      <ReactRecord
        ...
        onData={onData}
      />
      {isStart ? <div className={classes.recordText}>녹음중입니다.</div> : null}
            ...
            <DialogContent>
              <DialogContentText>
                <div className={classes.content} style={{ margin: 0 }}>
                  입력 :
                  <span className={classes.rightWord}>{spokenSentence}</span>
                </div>
